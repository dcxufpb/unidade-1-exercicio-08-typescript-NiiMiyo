...
test("Complemento vazio", () => {
	...
	expect(dados_loja_objeto(complementoVazio)).toBe(
...
test("Observação vazia", () => {
	...
	expect(dados_loja_objeto(observacaoVazia)).toBe(
...
test("Número zero e complemento vazio", () => {
	...
	expect(dados_loja_objeto(numeroZeroComplementoVazio)).toBe(
...
test("Número zero, complemento e bairro vazios", () => {
	...
	expect(dados_loja_objeto(numeroZeroComplementoVazioBairroVazio)).toBe(
...
test("Exercício 2 - customizado", () => {
	...
	let nome_loja = "Top 10 nomes de lojas";
	let logradouro = "Rua Tchurusbango Tchurusmago";
	let numero = 13;
	let complemento = "Do lado da casa vizinha";
	let bairro = "Bairro do Limoeiro";
	let municipio = "São Paulo";
	let estado = "SP";
	let cep = "08090-284";
	let telefone = "(11) 4002-8922";
	let observacao = "Entre o Campinho e a Lua de Baixo";
	let cnpj = "43.745.249/0001-39";
	let inscricao_estadual = "564.213.199.866";
	...
	expect(dados_loja_objeto(loja_customizada)).toBe(expected);
});
